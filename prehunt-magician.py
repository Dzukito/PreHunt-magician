...
if response.status_code == 200:
    ...
    with open('output.csv', 'w', newline='') as csvfile:
        ...
        for grupo, tecnicas in dic_grupo_tec.items():
            for tecnica in tecnicas:
                writer.writerow([tecnica, dic_id_grupo[grupo][0],str(dic_tecnicas_tacticas[tecnica]).replace("[","").replace("]","").replace("'",""),dic_tecnicas_nombre[tecnica],dic_tecnicas_description[tecnica]])
